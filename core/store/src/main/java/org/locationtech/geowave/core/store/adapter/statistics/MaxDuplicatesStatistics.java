...
public class MaxDuplicatesStatistics<T> extends
  ...
  public MaxDuplicatesStatistics(final short internalDataAdapterId, final String indexName) {
    super(internalDataAdapterId, STATS_TYPE, indexName);
  }
  ...
  public byte[] toBinary() {
    ...
    VarintUtils.writeUnsignedInt(maxDuplicates, buf);
    return buf.array();
  }
  ...
  public void merge(final Mergeable merge) {
    if ((merge != null) && (merge instanceof MaxDuplicatesStatistics)) {
      maxDuplicates = Math.max(maxDuplicates, ((MaxDuplicatesStatistics) merge).maxDuplicates);
    }
  }
