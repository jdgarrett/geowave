...
public class FieldVisibilityCount<T> extends
  ...
  public FieldVisibilityCount(final short internalDataAdapterId, final String indexName) {
    this(internalDataAdapterId, indexName, new HashMap<>());
  }
  ...
  @Override
  public DataStatistics<T, Map<ByteArray, Long>, IndexStatisticsQueryBuilder<Map<ByteArray, Long>>> duplicate() {
  ...
  protected Object resultsValue() {
    ...
    for (final Entry<ByteArray, Long> entry : countsPerVisibility.entrySet()) {
      retVal.put(entry.getKey().getString(), entry.getValue());
    }
    return retVal;
  }
}
